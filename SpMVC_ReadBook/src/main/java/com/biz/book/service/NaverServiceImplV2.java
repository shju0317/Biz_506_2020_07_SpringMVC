...
import com.biz.book.model.BookList;
...
public class NaverServiceImplV2 extends NaverServiceImplV1{
	...
	public List<BookVO> getNaverList(String queryURL) {
		...
		// queryURL(요청주소 포함된) 문자열을 URI 객체로 만들어
		// http 프로토콜에서 사용할 수 있도록 만드는 클래스
		// 기존 URL클래스가 있으나, 새로운 기능을 수행하기 위해서
		// 별도로 URI클래스를 만들어 놨으며 많은 기능들이 추가되어 있다.
		 ...
		 * 복잡하게 만들어야 했다.
		 * 
		 * spring에서 외부API를 사용하여 데이터를 가져올 일이 점점 많아지면서
		...
		ResponseEntity<BookList> bookList = null;
		
		bookList = restTemp.exchange(restURI, HttpMethod.GET, entity, BookList.class);
