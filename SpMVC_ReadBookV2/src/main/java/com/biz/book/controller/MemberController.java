 ...
 * 서버의 메모리에 memberVO라는 이름으로 객체변수를 마련해 놓는다.
 * 이 변수는 서버가 재시작하더라도 유지되는 성질이 있어서
 * 클라이언트에서 URL request를 수행했을 때 메모리에 계속 유지된다.
...
public class MemberController {
	...
	public String mypage(@ModelAttribute("memberVO") UserDetailsVO userVO, Authentication authProvider, Model model) {
		 ...
		 *  spring security를 통과형 login이 인가된 사용자의 정보는
		 *  현재 method가 호출될 때 spring security의 filter chain에 의해서
		 *  method의 매개변수로 설정된 Authentication 클래스로 선언된 객체에 담겨서 전달된다.
		 *  
	...
	public String id_check(String username) {
		...
		// 문법오류가 발생하면 구체적으로 memberService 메서드를 구현하는 방법 
		UserDetailsVO userVO = memberService.findById(username);
	...
	public UserDetailsVO userInfo(Principal principal, Authentication authentication, @AuthenticationPrincipal @ModelAttribute("memberVO") UserDetailsVO userVO, Model model) {
