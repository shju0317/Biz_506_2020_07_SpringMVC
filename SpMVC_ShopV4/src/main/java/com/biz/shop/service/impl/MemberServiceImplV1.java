...
public class MemberServiceImplV1 implements MemberService{
	 ...
	 * 6. USER권한: ADMIN권한으로 접근할 수 있는 부분은 사용이 불가하다.
	...
	public int insert(MemberVO memVO) {
		 ...
		 * 이 비교연산식은 문법적,논리적으로 아무런 문제가 없는 코드이다.
		 *
		 * But 실제상황에서 JDBC를 통해서 서버에 원격연결을 하는 상황에서는
		 * Count값이 실제로는 0이지만 0이 아닌 -1 등의 값이 될 수도 있다.
		 * 데이터가 없을 때, 0이거나 -1인 경우가 된다면
		 * if(count==0 || count == -1) 또는
		 * if(count <= 0) 과 같은 비교연산식이 만들어저야 한다.
		 * 
