...
public class OrderController {
	 ...
	 * 이미 생성되어 Container에 보관중인 OrderSerivceImplV1 객체를
	...
	/*
	 * browser에서 order request를 실행할 때(요청할 때)
	 * ?변수1=값&변수2=값&변수3=값 형식으로 query를 나열하여 요청할 때
	...
	public String getOrder(String seq, String name, Model model) {
		...
		try {
			...
			System.out.println("SEQ값을 받지못함 or 잘못된 SEQ를 받음");
		}
		 ...
		 * 클라이언트(요청한 곳)으로 응답을 해라
		 * 이때 model에 담겨있는 데이터를 rendering할 때 사용하라
		 */
	...
	public String insert(String o_num, String o_date, String o_cnum, String o_pcode, String o_price, String o_qty, Model model) {
