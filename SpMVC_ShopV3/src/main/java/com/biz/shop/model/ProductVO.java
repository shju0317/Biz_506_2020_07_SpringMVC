...
public class ProductVO {
	...
	// DB연동을 할때 CHAR(1) TinyInt형으로 선언된 칼럼과 연동하기 위해
	// 사용하는 type이 typ 0~255, -128~127까지의 값만을 저장하는 데이터
	// 메모리 공간 크기가 1byte(8bit)
	// int형은 java에서 4byte크기를 사용
	// String형은 wrapper클래스이다보니 실제적으로 상당히 큰
	// 메모리 공간을 차지한다.
	// 단순히 flag와 같은 데이터를 취급할 때는 char 또는 byte형으로 사용하면
	// 메모리를 절약할 수 있다.
	
	/*
	 * DB p_not_use 칼럼의 값이 NULL(IS NULL)이면 정상적인 데이터이고
	 * NULL이 아니면(IS NOT NULL)이면 삭제된 데이터로 취급하기로 하였다
	 * Update를 수행할 때 VO의 칼럼에 값이 null이면
	 * jdbcType.VARCHAR, config에 수행한 설정때문에
	 * DB에 Update할 때 NULL이 아닌 다른 값이 저장되어 버린다.
